...
export function CalendarView({ project, onUpdateProject }) {
  ...
  return (
    <div className="px-6 py-10 pb-32">
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-stone-700 bg-white dark:bg-zinc-800">
        <div
          className="min-w-max"
          style={{ display: 'grid', gridTemplateColumns: `140px repeat(${dayCount}, minmax(72px, 1fr))` }}
        >
          {/* header row */}
          <div className="sticky left-0 z-10 bg-white dark:bg-zinc-800 border-b border-r border-slate-200 dark:border-stone-700 px-3 py-2 text-xs font-semibold text-slate-400 dark:text-stone-500 uppercase tracking-wide">
            Status
          </div>
          {days.map((d, i) => {
          ...
          {STAGES.map(stage => {
            ...
            return (
              <div key={stage.id} className="contents">
                {/* stage label cell */}
                <div className="sticky left-0 z-10 bg-white dark:bg-zinc-800 border-b border-r border-slate-100 dark:border-stone-700 px-3 py-3 flex items-center">
        ...
        <section className="mt-8">
          ...
          <div className="flex flex-wrap gap-2">
            {tasksNoDate.map(task => {
              ...
              return (
                <button key={task.id} onClick={() => setEditing(task)}
                  className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition hover:shadow-sm ${tc.stripe} bg-white dark:bg-zinc-800 border-slate-200 dark:border-stone-700 text-slate-700 dark:text-stone-200`}>
                  {tc.icon} {task.title}
                </button>
              );
            })}
